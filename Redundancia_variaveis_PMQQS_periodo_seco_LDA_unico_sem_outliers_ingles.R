...
for(i in 1:39){
  base_entropia_marginal_PMQQS[[i]] = read_excel(path = "D:/UFMG/OneDrive/Doutorado - UFMG/Tese/Resultados/Eixo I/Análise alternativa sem outliers e correção do LDA/Análise - inglês/Entropia/Resultados/Tabelas/Entropia_marginal_PMQQS_periodo_seco_LDA_unico_sem_outliers_ingles.xlsx", 
                                                 sheet = i)
}
...
for(i in 1:39){
  base_entropia_max_PMQQS[[i]] = read_excel(path = "D:/UFMG/OneDrive/Doutorado - UFMG/Tese/Resultados/Eixo I/Análise alternativa sem outliers e correção do LDA/Análise - inglês/Entropia/Resultados/Tabelas/Entropia_max_PMQQS_periodo_seco_LDA_unico_sem_outliers_ingles.xlsx", 
                                                 sheet = i)
}
...
#Truncando valores de redundância
...
#Criando colunas para identificação das estações
library(dplyr)
redundancia_var_agrupadas_PMQQS = mutate(.data = redundancia_var_agrupadas_PMQQS, "Estações PMQQS" = row.names(redundancia_var_agrupadas_PMQQS))
redundancia_var_agrupadas_PMQQS = select(.data = redundancia_var_agrupadas_PMQQS, `Estações PMQQS`, everything())

#Exportando documento Excel da determinação da redundância das variáveis da rede PMQQS
library(openxlsx)
redundancia_var_PMQQS_Excel = write.xlsx(x = redundancia_var_agrupadas_PMQQS, file = "D:/UFMG/OneDrive/Área de Trabalho/Redundancia_var_PMQQS_periodo_seco_LDA_unico_sem_outliers_ingles.xlsx")
