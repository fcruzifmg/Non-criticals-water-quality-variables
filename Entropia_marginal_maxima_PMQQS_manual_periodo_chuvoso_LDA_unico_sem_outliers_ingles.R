...
entropia_max_modify = function(base){
  ...
  entropia_max = data.frame(sort(x = entropia_list_df))
  entropia_max = entropia_max[c(90),]
  return(entropia_max)
}

#-------------------------------------
#Função para quebra de uma base no formato dataframe para lista (lista de variáveis, sendo cada objeto da lista um dataframe de variável de QA)
...
for(i in 1:39){
  Base_PMQQS_entropia_max[[i]] = read_excel(path = "D:/UFMG/OneDrive/Doutorado - UFMG/Tese/Resultados/Eixo I/Análise alternativa sem outliers e correção do LDA/Análise - inglês/Entropia/Resultados/Tabelas/Base_PMQQS_sem_var_entropia_nula_periodo_chuvoso_LDA_unico_sem_outliers_ingles.xlsx",
                                            sheet = i)
}
...
#Retornando informações da base de estações sem variáveis de entropia nula
...
library(openxlsx)
write.xlsx(Entropia_max_base_PMQQS_df, file = "D:/UFMG/OneDrive/Área de Trabalho/Entropia_max_PMQQS_periodo_chuvoso_LDA_unico_sem_outliers_ingles.xlsx")
